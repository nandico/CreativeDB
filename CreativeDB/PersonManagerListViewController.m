//
//  PersonManagerListViewController.m
//  CreativeDB
//
...
@interface PersonManagerListViewController ()
...
@property (nonatomic, strong) NSTableColumn *portfolioURLColumn;

@property (nonatomic, strong) NSMutableArray *items;

@end

@implementation PersonManagerListViewController
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(updateList:)
                                                     name:PERSON_MANAGER_UPDATE_LIST object:nil];
...
- (void) createList
{
    ...
    _portfolioURLColumn = [[NSTableColumn alloc] initWithIdentifier:@"portfolioURL"];
    
    [_nameColumn.headerCell setStringValue:@"Name"];
    [_countryColumn.headerCell setStringValue:@"Country"];
    [_portfolioURLColumn.headerCell setStringValue:@"Portfolio URL"];
    
    [_nameColumn setWidth:COMPLETE_VIEW_CONTAINER_LIST_WIDTH / 3.0f];
    [_countryColumn setWidth:COMPLETE_VIEW_CONTAINER_LIST_WIDTH / 3.0];
    [_portfolioURLColumn setWidth:COMPLETE_VIEW_CONTAINER_LIST_WIDTH / 3.0];
    
    [_tableView addTableColumn:_nameColumn];
    [_tableView addTableColumn:_countryColumn];
    [_tableView addTableColumn:_portfolioURLColumn];
...
- (void)tableViewSelectionDidChange:(NSNotification *) notification
{
    ...
    if( firstIndex )
    {
        PersonModel *item = [_items objectAtIndex:[firstIndex integerValue]];
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    NSTextField *cell = [tableView makeViewWithIdentifier:@"PersonView" owner:self];
    
    if (nil == cell) {
        cell = [[NSTextField alloc] initWithFrame:CGRectZero];
        cell.identifier = @"PersonView";
        [cell setBezeled:NO];
    ...
    PersonModel *item = [_items objectAtIndex:row];
    ...
    {
        if( item.country.name ) [cell setStringValue:item.country.name];
    }
