//
//  ClientManagerListViewController.m
//  CreativeDB
//
...
@interface ClientManagerListViewController ()
...
@implementation ClientManagerListViewController
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(updateList:)
                                                     name:CLIENT_MANAGER_UPDATE_LIST object:nil];
...
- (void)tableViewSelectionDidChange:(NSNotification *) notification
{
    ...
    if( firstIndex )
    {
        ClientModel *item = [_items objectAtIndex:[firstIndex integerValue]];
        NSDictionary *updateMessage = [NSDictionary dictionaryWithObject:item forKey:MLE_FIELDSET_MODEL_ITEM];
        
        [[NSNotificationCenter defaultCenter] postNotificationName:CLIENT_MANAGER_UPDATE_VIEW
                                                            object:self
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    NSTextField *cell = [tableView makeViewWithIdentifier:@"ClientView" owner:self];
    
    if (nil == cell) {
        cell = [[NSTextField alloc] initWithFrame:CGRectZero];
        cell.identifier = @"ClientView";
        [cell setBezeled:NO];
    ...
    ClientModel *item = [_items objectAtIndex:row];
