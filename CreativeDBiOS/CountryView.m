...
- (void) updateData
{
    CountryModel *selectedCountry = [self.dataSource selectedCountry];
    
    if( !selectedCountry ) return;
    
    self.thumb.userInitials.text = selectedCountry.iso3;
    self.name.text = selectedCountry.name;
    self.country.text = selectedCountry.name;
    self.flag.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@.png", [selectedCountry.iso lowercaseString] ]];
    
    self.globalStats.text = @"GLOBAL";
    self.globalScore.text = [NSString stringWithFormat:@"%@", [self addSuffixToNumber:[selectedCountry.rankingGlobal longValue]]];
    
    [self.countryStats setHidden:YES];
    [self.countryScore setHidden:YES];
