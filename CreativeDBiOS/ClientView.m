...
- (void) updateData
{
    ClientModel *selectedClient = [self.dataSource selectedClient];
    
    if( !selectedClient ) return;
    
    self.thumb.userInitials.text = [self.thumb extractInitials:selectedClient.name];
    self.name.text = selectedClient.name;
    self.country.text = selectedClient.country.name;
    self.flag.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@.png", [selectedClient.country.iso lowercaseString] ]];
    
    self.globalStats.text = @"GLOBAL";
    self.globalScore.text = [NSString stringWithFormat:@"%@", [self addSuffixToNumber:[selectedClient.rankingGlobal longValue]]];
    
    self.countryStats.text = @"COUNTRY";
    self.countryScore.text = [NSString stringWithFormat:@"%@", [self addSuffixToNumber:[selectedClient.rankingCountry longValue]]];
